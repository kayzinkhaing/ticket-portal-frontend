  ...
  <div class="animate-fadein p-8 text-white max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
      ...
      <section class="lg:col-span-2 glass-card rounded-3xl p-8">
        ...
        <div v-if="!loading && !targets.length" class="text-gray-500">No SLA data found.</div>
      </section>
      ...
      <aside class="space-y-8">
        <div class="glass-card rounded-3xl p-6">
          ...
          <div class="space-y-6">
            <SlaToggle label="Internal Slack Alert" description="Send notification to #support-ops when ticket is at 80% threshold." v-model="settings.slackAlert" />
            <SlaToggle label="Manager Escalation" description="Notify lead agent via email if target is exceeded." v-model="settings.managerEscalation" />
            <SlaToggle label="Client Delay Notice" description="Automatically inform client of high-traffic delays." v-model="settings.clientNotice" disabled />
          </div>
          <button class="w-full mt-8 py-3 bg-violet-600/10 text-violet-400 text-[10px] font-bold uppercase tracking-[0.2em] rounded-xl hover:bg-violet-600/20 transition-all border border-violet-500/20">
            Add Custom Rule
          </button>
        </div>
...
const getColor = (name) => {
  switch (name.toLowerCase()) {
    case 'critical': return { bg: 'bg-[#ff8a8a]', text: 'text-[#ff8a8a]' };
    case 'high': return { bg: 'bg-[#8a94ff]', text: 'text-[#8a94ff]' };
    case 'medium': return { bg: 'bg-[#5eead4]', text: 'text-[#5eead4]' };
    case 'low': return { bg: 'bg-gray-500', text: 'text-gray-500' };
...
const fetchPriorities = async () => {
  ...
  try {
    ...
    targets.value = priorities.map(item => {
