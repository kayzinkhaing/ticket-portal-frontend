        ...
        <div :class="showCommentsPanel ? 'lg:col-span-7' : 'lg:col-span-8'" class="space-y-8">
          ...
          <div class="glass-card rounded-xl overflow-hidden border border-white/10">
            ...
            <div class="p-6 bg-[#161824]">
              <textarea v-model="newCommentText" rows="4" placeholder="Update the client..." class="w-full bg-transparent border-none outline-none text-gray-300 text-sm focus:ring-0"></textarea>
    ...
    <div v-if="replyingTo" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
      <div class="glass-card w-full max-w-lg p-8 rounded-3xl border border-white/10 shadow-2xl">
        ...
        <div class="flex gap-4">
          ...
          <button @click="submitComment(replyingTo.id)" class="flex-1 bg-violet-600 py-3 rounded-xl font-bold">Post Reply</button>
        </div>
      </div>
    </div>
  </main>
...
const confirmReassign = async () => {
  ...
  try {
    ...
    triggerToast();
  } catch (err) { alert("Assignment failed"); } finally { assigning.value = false; }
};
...
const openReplyModal = (comment) => { replyingTo.value = comment; };
